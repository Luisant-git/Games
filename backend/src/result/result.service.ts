...
export class ResultService {
  ...
  private checkIfBetWins(gameplay: any, boards: any): boolean {
    ...
    return betNumbers.some(
      (betNum) => betNum.toString() === winningNumber.toString(),
    );
  }

  private async calculateWinAmount(
    gameplay: any,
    boards: any,
  ): Promise<number> {
    ...
    if (
      gameplay.betType === 'TRIPLE_DIGIT' ||
      gameplay.betType === 'FOUR_DIGIT'
    ) {
    ...
    // Check if bet wins
    const isWinning =
      gameplay.betType === 'TRIPLE_DIGIT' || gameplay.betType === 'FOUR_DIGIT'
    ...
    if (
      ...
      console.log(
        `Multi-digit calculation: betType=${gameplay.betType}, betNum=${betNum}, winNum=${winNum}`,
      );

      if (betNum === winNum) {
        const winAmount = (game.winningAmount || 0) * qty;
        console.log(
          `Exact match: winAmount=${winAmount} (${game.winningAmount} * ${qty})`,
        );
        return winAmount;
      }
      ...
      if (gameplay.betType === 'TRIPLE_DIGIT') {
        ...
        if (betNum.slice(-2) === winNum.slice(-2)) {
          ...
          console.log(
            `Triple-digit 2-digit match: ${betNum.slice(-2)} == ${winNum.slice(-2)}, winAmount=${winAmount}`,
          );
          return winAmount;
        }
        ...
        if (betNum.slice(-1) === winNum.slice(-1)) {
          ...
          console.log(
            `Triple-digit 1-digit match: ${betNum.slice(-1)} == ${winNum.slice(-1)}, winAmount=${winAmount}`,
          );
          return winAmount;
        }
      }
      ...
      if (gameplay.betType === 'FOUR_DIGIT') {
        ...
        if (betNum.slice(-3) === winNum.slice(-3)) {
          ...
          console.log(
            `Four-digit 3-digit match: ${betNum.slice(-3)} == ${winNum.slice(-3)}, winAmount=${winAmount}`,
          );
          return winAmount;
        }
        // Check last 2 digits match
        if (betNum.slice(-2) === winNum.slice(-2)) {
          const winAmount = (game.doubleDigitMatching || 0) * qty;
          console.log(
            `Four-digit 2-digit match: ${betNum.slice(-2)} == ${winNum.slice(-2)}, winAmount=${winAmount}`,
          );
          return winAmount;
        }
        // Check last digit match
        if (betNum.slice(-1) === winNum.slice(-1)) {
          const winAmount = (game.singleDigitMatching || 0) * qty;
          console.log(
            `Four-digit 1-digit match: ${betNum.slice(-1)} == ${winNum.slice(-1)}, winAmount=${winAmount}`,
          );
          return winAmount;
        }
      }
    ...
    console.log(
      `Final win amount: ${finalWinAmount} (${game.winningAmount} * ${qty})`,
    );
  ...
  async publishResult(id: number) {
    ...
    if (existingPublished) {
      throw new BadRequestException(
        'A result is already published for this date and time',
      );
    }
    ...
    for (const history of gameHistories) {
      ...
      if (totalWinAmount > 0) {
        ...
        // Agent wins - add bet amount minus commission to agent wallet (commission already added during gameplay)
        const totalBetAmount = history.gameplay.reduce(
          (sum, gameplay) => sum + gameplay.amount,
          0,
        );
        const netAmountForAgent =
          totalBetAmount - (history.agentCommission || 0);
        console.log(
          `Agent wins: Total bet ₹${totalBetAmount}, Commission already paid ₹${history.agentCommission || 0}, Net amount ₹${netAmountForAgent}`,
        );

        if (history.agentId && netAmountForAgent > 0) {
          await this.prisma.agentWallet.update({
            where: { agentId: history.agentId },
            data: { balance: { increment: netAmountForAgent } },
          });
        }
      }
    }
